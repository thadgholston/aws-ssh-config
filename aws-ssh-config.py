...
def build_argument_parser(): 
    ...
    parser.add_argument('--tags', action='append', default = [], help='A comma-separated list of tag names to be considered for concatenation. If omitted, all tags will be used.')
    parser.add_argument('--user', help='Override the ssh username for all hosts')
    parser.add_argument("--tag-filter", action='append', dest='tag_filters', default = [], help='tags to exclusively use for building config file')
    parser.add_argument('--proxy-server-name', help='name of server to use a proxy')
    parser.add_argument('--region', default='us-east-1', help='Which regions must be included. If omitted, all regions are considered')
    parser.add_argument("--substitue", action='append', dest='words_to_substitue', default = [], help='word to substitue in tag value used to build host name')
    parser.add_argument("--remove", action='append', dest='words_to_remove', default = [], help='word to remove from tag value used to build host name')
...
def retreive_running_linux_instances(ec2, additional_filters = []):
    ...
    additional_filters = [create_dict_of_filters(tag_and_values) for tag_and_values in additional_filters]
    filters = required_filters + additional_filters
    running_instances = ec2.instances.filter(Filters=filters)
    return [instance for instance in running_instances if instance.platform != 'windows']
...
def generate_host_name(instance, hostname_tags, substitution_list, remove_list, prefix):
...
def generate_config_entry(instance, use_private_ip, no_identities_only, strict_hostkey_checking, hostname_tags, substitution_list, remove_list, prefix, proxy_server = None):
    if use_private_ip and instance.private_ip_address:
        ...
        sys.stderr.write('Cannot lookup ip address for instance %s' % instance.id)
        return

    host_name = generate_host_name(instance, hostname_tags, substitution_list, remove_list, prefix)
...
def main():
    ...
    try:
        ...
        print("Invalid profile provided: \"" + profile + "\". Check your ~/.aws/credentials file to confirm the name of your profile.")
        return
    ...
    if proxy_server_name and not proxy_server:
        ...
        for instance in potential_proxy_servers:
            ...
            if 'Name' in tag_map:
                ...
                print('  * ' + instance.id + ' (ERROR retrieving name. Displaying instance id)')
        return
    ...
    if proxy_server:
        proxy_host_name = generate_host_name(proxy_server, hostname_tags, substitution_list, remove_list, prefix)
        generate_config_entry(proxy_server, use_private_ip, no_identities_only, strict_hostkey_checking, hostname_tags, substitution_list, remove_list, prefix)

    for instance in instances:
        generate_config_entry(instance, use_private_ip, no_identities_only, strict_hostkey_checking, hostname_tags, substitution_list, remove_list, prefix, proxy_host_name)

        # if arguments.private and instance.private_ip_address:
        #   ip_address = instance.private_ip_address
        # elif not arguments.private and instance.public_ip_address:
        #   ip_address = instance.public_ip_address
        # elif not arguments.private and instance.private_ip_address:
        #   ip_address = instance.private_ip_address
        # else:
        #   sys.stderr.write('Cannot lookup ip address for instance %s' % instance.id)
        #   return
        ...
        # if proxy_server:
        #   print('    Proxycommand ssh -W %h:%p ' + generate_host_name(proxy_server, hostname_tags, substitution_list, remove_list, prefix))

        # if not arguments.no_identities_only:
        #   # ensure ssh-agent keys don't flood when we know the right file to use
        #   print('    IdentitiesOnly yes')
        # if not arguments.strict_hostkey_checking:
        #   print('    StrictHostKeyChecking no')
        #   print()
